...
# min init
irc_n_high=0
title="%{F${color_head} B${color_sec_b2}}${sep_right}%{F${color_title_fore} B${color_sec_b2}%{T2} ${icon_prog} %{F${color_sec_b2} B-}${sep_right}%{F- B- T1}"
...
while read -r line ; do
  case $line in
    SYS*)
      ...
      date="%{F${color_bdate}}${sep_left}%{F${color_icon} B${color_bdate}} %{T2}${icon_clock}%{F- T1} ${date}"

      # time
      time="%{F${color_time}}${sep_left}%{F#ffffffff B${color_time}} ${sys_arr[3]} %{F- B-}"

      # cpu
      if [ ${sys_arr[4]} -gt ${cpu_alert} ]; then
        cpu_cback=${color_red}; cpu_cicon=${color_fore}; cpu_cfore=${color_fore};
      else
      ...
      # disk /
      diskr="%{F${color_c}}${sep_left}%{F${color_icon} B${color_c}} %{T2}${icon_hd}%{F- T1} ${sys_arr[6]}%"
      # disk home
      ...
      if [ "${sys_arr[8]}" == "down" ]; then
        ...
        wlan_cicon=${color_disable}; wlan_cfore=${color_disable};
      else
      ...
      wland="%{F${color_b}}${sep_left}%{F${color_b_fore} B${color_b}} %{T2}${icon_dl}%{F${color_b_fore} T1} ${wland_v}"
      wlanu="%{F${color_b_fore}}${sep_l_left} %{T2}${icon_ul}%{F${color_b_fore} T1} ${wlanu_v}"
      # eth
      ethd_v=${sys_arr[10]}; ethu_v=${sys_arr[11]};
	  temp_v=${sys_arr[12]};
      eth_cback=${color_sec_b1}; eth_cicon=${color_icon}; eth_cfore=${color_fore};

      ethd="%{F${color_blue}}${sep_left}%{F${color_rpm_fore} B${color_blue}} %{T2}${icon_temp}%{F${color_rpm_fore} T1} ${ethd_v} ${temp_v}C"
      ethu="%{F${color_bat}}${sep_left}%{F${color_red} B${color_bat}} %{T2}${icon_bat}%{F${color_red} T1} ${ethu_v}"
      ;;
    ...
    MPD*)
      ...
      mpd="%{F${color_sec_b2}}${sep_left}%{B${color_sec_b2}}%{F${color_a}}${sep_left}%{F${color_icon} B${color_a}} %{T2}${icon_music}%{F${color_fore} T1}  ${song}"
      ;;
    WSP*)
      # I3 Workspaces
    ...
    WIN*)
      ...
      title="%{F${color_head} B${color_sec_b2}}${sep_right}%{F${color_fore} B${color_sec_b2} %{T2} ${icon_prog} %{F${color_prog} B-}${sep_right}%{F- B- T1} ${title}"
      ;;
  esac

  # And finally, output
  printf "%s\n" "%{l}${wsp}${title} %{r}${mpd}${stab}${wland}${stab}${wlanu}${stab}${vol}${stab}${cpu}${stab}${mem}${stab}${ethd}${stab}${ethu}${stab}${date}${stab}${time}"
  #printf "%s\n" "%{l}${wsp}${title}"
done
